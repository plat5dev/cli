...
`plat5_version` (default `v0.1.6`) pins runtime GHCR tags. With Auth enabled, `auth.version` / `AUTH_VERSION` (default `v0.1.4`) pins `ghcr.io/plat5dev/auth` independently.
...
plat5_version: v0.1.6                  # runtime GHCR tag

auth:
  enabled: false
  # version: v0.1.4                    # Auth image pin when enabled (AUTH_VERSION)
